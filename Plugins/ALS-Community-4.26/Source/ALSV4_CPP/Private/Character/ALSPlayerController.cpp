...
void AALSPlayerController::OnPossess(APawn* NewPawn)
{
	...
	CombatSystem = Cast<UCombatSystem>(GetPawn()->GetComponentByClass(UCombatSystem::StaticClass()));

	if (CombatSystem)
		CombatSystem->SetPlayer(PossessedCharacter);
...
void AALSPlayerController::CounterAction(const FInputActionValue& Value)
{
	if (CombatSystem)
	{
		CombatSystem->Counter();
	}
}
