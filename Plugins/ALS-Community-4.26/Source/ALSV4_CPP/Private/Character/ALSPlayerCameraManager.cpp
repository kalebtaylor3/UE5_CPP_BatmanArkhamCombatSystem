...
void AALSPlayerCameraManager::UpdateViewTargetInternal(FTViewTarget& OutVT, float DeltaTime)
{
	...
	// 2) **Then** apply your manual offset in camera-local space
	{
		// Rotate your offset vector by the final camera rotation
		const FVector OffsetWorld = OutVT.POV.Rotation.RotateVector(ManualCameraOffset);
		// Shift the camera location
		OutVT.POV.Location += OffsetWorld;
	}
}
